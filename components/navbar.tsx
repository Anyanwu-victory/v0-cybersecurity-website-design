    ...
    <nav className="sticky top-0 z-50 w-full border-b border-white/10 bg-background/80 backdrop-blur-md">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2">
          <div className="relative flex h-10 w-10 items-center justify-center rounded-lg bg-[#E11D2E]/20 neon-glow-red">
            <Shield className="h-6 w-6 text-[#E11D2E]" />
          </div>
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                ...
                pathname === link.href ? "text-[#E11D2E]" : "text-muted-foreground",
              )}
            >
              {link.name}
            </Link>
